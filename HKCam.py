...
class HKCam(object):
    ...
    def start_play(self,):
        ...
        self.preview_info.dwStreamType = 0  # 0-主码流，1-子码流，2-码流3，3-码流4，以此类推
        self.preview_info.dwLinkMode = 0  # 0- TCP方式，1- UDP方式，2- 多播方式，3- RTP方式，4-RTP/RTSP，5-RSTP/HTTP
        self.preview_info.bBlocked = 0  # 0- 非阻塞取流，1- 阻塞取流
    ...
    def read(self):
        while self.n_stamp==self.last_stamp:
            # print(f'n_stamp:{self.n_stamp}')
            # print(f'last_stamp:{self.last_stamp}')
            continue
...
if __name__=="__main__":
    ...
    while True:
        t0 =time.time()
        ret,img = hkclass.read()
        if type(img) is not np.ndarray:
            continue
        print(img.shape, '----',n)
        hkclass.show_img(img)
        key = cv2.waitKey(10)
        if (key & 0xFF == ord('q')):
            break
        n += 1
